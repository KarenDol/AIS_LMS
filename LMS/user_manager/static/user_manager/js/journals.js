document.addEventListener('DOMContentLoaded', function() {
    ...
    //Edit existing honor
    function allow_edit(journal_id){
        //Change the form submission address
        form.action = `/edit_journal/${journal_id}/`;
    ...
    function addJournal(journal){
        ...
        timelineItem.innerHTML = `
            <h3 class="timeline-title">${journal.title}</h3>

            <div class="icon-group">
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" class="edit" data-id="${journal.id}">
                ...
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" class="trash" data-id="${journal.id}">>
            ...
            <p class="timeline-date">
                ${formatDateToRussian(journal.date)} 
                <span class="timeline-event">${journal.description}</span>
            </p>
        `;
    ...
    function deleteJournal(journalId) {
        ...
        .then(response => {
            if (response.ok) {
                ...
                journal = journals.filter(journal => journal.id !== parseInt(journalId));
                populateJournals(); // Re-render the honors list
    ...
    function attachEditListeners() {
        ...
        editIcons.forEach(icon => {
            icon.addEventListener('click', function () {
                const journalId = this.getAttribute('data-id');
                const honorData = journals.find(journal => journal.id === parseInt(journalId));
                const { title, date, description } = honorData;

                allow_edit(journalId);
    ...
    function addInput(){
        ...
        timelineInput.innerHTML = `
            <h3 class="timeline-title"><input type="text" class="title" id="title-input" name="title" maxlength="100" placeholder="Разговор с родителями..."></h3>
            <p class="timeline-date">
                <input type="month" class="date" name="date" value="2024-12" min="2022-01"></i>
                <span class="timeline-event"><input type="text" class="event" id="event-input" name="description" maxlength="200" placeholder="Обсуждена необходимость доп. занятий по математике..."></span>
            </p>
        `;
